<template>
	<view class="wrapper">
		...
		<view style="width: 100%;" class="u-flex">
			<view class="sign_left" @click.stop="retDraw">
				重新签名
			</view>
			<view class="sign_right" @click.stop="uploadCanvasImg">
				确认签名
			</view>
		</view>
	</view>
...
<script>
	export default {
		...
		onLoad(option) {
			let that = this
			that.signatureKey = option.signatureKey || ''
					...
					success(res) {
						...
						uni.uploadFile({
							...
							success(res) {
								...
								globalData.signNameImg = data
								globalData.signatureKey = that.signatureKey
								uni.setStorageSync('signNameImg', data)
								uni.navigateBack({
									delta: 1
								});
							}
