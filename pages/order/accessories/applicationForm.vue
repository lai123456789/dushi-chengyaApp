<template>
	<view>
		...
		<u-card :body-style="{padding: '0rpx 32rpx'}">
			...
			<u-form slot="body">
				...
				<u-form-item label="物流公司" :label-width="label_width">
					<view class="u-content-color">{{dataList.pjjswlgs}}</view>
				</u-form-item>
				<u-form-item label="物流单号" :label-width="label_width">
					<view class="u-content-color">{{dataList.pjjswldh}}</view>
				</u-form-item>
				<u-form-item label="发货日期" :label-width="label_width">
					<view class="u-content-color">{{dataList.fhrq}}</view>
				</u-form-item>
				<u-form-item label="发货方式" :label-width="label_width">
					<view class="u-content-color">{{dataList.fhfs}}</view>
				</u-form-item>
			</u-form>

		</u-card>
		<u-card>
			<view slot="head">配件申请明细</view>
			<view slot="body">
				<view class="u-flex u-border-bottom u-p-t-10 u-p-b-20" v-for="(item,index) in dataList.pjsqmx" :key="item.id"
				 v-show="true">
					...
					<view class="u-m-l-20 u-flex-1">
						...
						<!-- <u-cell-group>
								<u-cell-item icon="setting-fill" title="个人设置" :border-bottom="false"></u-cell-item>
								<u-cell-item icon="integral-fill" title="会员等级" value="新版本" :border-bottom="false"></u-cell-item>
							</u-cell-group> -->

					</view>

				</view>
				<view class="u-m-t-20 u-flex u-row-right"><text>总计：</text><text class="u-type-error">￥{{form.pjzje}}</text></view>
			</view>
		</u-card>
		<u-card v-if="cIndex === 0">
			<view slot="head">工单办理</view>
			<u-form slot="body" :model="form" ref="uForm">
				<u-form-item label="办理方式" label-width="200">
					<u-radio-group v-model="handleWay" :wrap="true">
						<u-radio active-color="#2979ff" v-for="(item, index) in radiolist" :key="index" :name="item.way" @change="radioChange(item,index)">{{ item.way }}</u-radio>
					</u-radio-group>
				</u-form-item>
				<block v-if="showBan">
		...
		<u-card>
			<view slot="head">工单日志</view>
			<view slot="body">
				<u-read-more :toggle="true" close-text="查看完整工单日志" ref="uReadMore">
			...
			getpjsqinfo(value) {
				this.$u.post('/app/WorkOrder/getpjsqinfo', {
					...
					this.$nextTick(function() {
						...
						// this.form.ssq.address = res.result.data._spjdqssqjd_.split(',')[2]  //新增
